function decode(buffer) {
    ...
    const nulIndex = size - 1;
    let zeroOffset = 0;
    let zeroIndex = 0;
    while (zeroIndex !== nulIndex - zeroOffset - 1) {
        zeroIndex = nulIndex - zeroOffset - 1;
        
        if (zeroIndex >= nulIndex || zeroIndex < 0) {
            return Buffer.alloc(0);
        }
        
        if (buffer[zeroIndex] === nulIndex) {
            buffer[zeroIndex] = 0;
            break;
        } else {
            zeroOffset = buffer[zeroIndex];
            buffer[zeroIndex] = 0;
        }
    }
...
function crc8(crc, data) {
    const tblIdx = crc ^ data;
    return crcTable[tblIdx];
}
...
function check(buffer) {
    ...
    if (crc !== 0) {
        const error = "Tinbus CRC Failed : " + JSON.stringify(buffer);
        throw new Error (error);
    } else {
        const result = buffer.subarray(0, -1);  // Remove CRC from buffer
        return result; 
    }
}
...
function tinbus_decode(buffer){
    ...
    if (!Buffer.isBuffer(buffer)) {
        throw new TypeError("Must be a Buffer");
    }
    if (buffer.length == 0) {
...
let cobsm_decoded = decode(msg.payload)
